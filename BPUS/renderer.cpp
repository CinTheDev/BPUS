...
static void
draw_rect_pixel(h_Vector2Int p0, h_Vector2Int p1, u32 color) {
...
bool inside_oval(h_Vector2Int l, h_Vector2Int axis) {
...
static void
draw_oval_pixel(h_Vector2Int p, h_Vector2Int radius, u32 color) {
...
int sign_tri(h_Vector2Int p0, h_Vector2Int p1, h_Vector2Int p2) {
	return (p0.x - p2.x) * (p1.y - p2.y) - (p1.x - p2.x) * (p0.y - p2.y);
}

static void
draw_tri_pixel(h_Vector2Int p0, h_Vector2Int p1, h_Vector2Int p2, u32 color) {
	...
	for (int y = sy; y < by; y++) {
		...
		for (int x = sx; x < bx; x++) {
			d1 = sign_tri(h_Vector2Int(x, y), p0, p1);
			d2 = sign_tri(h_Vector2Int(x, y), p1, p2);
			d3 = sign_tri(h_Vector2Int(x, y), p2, p0);
...
void set_pixel(h_Vector2Int pos, h_Vector2Int pix, Image* img, float rotation, h_Vector2Int pivot) {
	h_Vector2 rot = pos.todouble() - pivot.todouble();
	rot.rotate(rotation);
	h_Vector2 point = pivot.todouble() + rot;
	h_Vector2Int pointint(floor(point.x), floor(point.y));
...
static void
draw_image_pixel(Image* image, h_Vector2Int offset, float scale, float rotation, h_Vector2Int pivot) {
	...
	for (int y = offset.y; y < height + offset.y; y++) {
		...
		for (int x = offset.x; x < width + offset.x; x++) {
			...
			set_pixel(h_Vector2Int(x, y), h_Vector2Int(px, py), image, rotation, pivot);
		}
	}
}

static void
draw_rect(h_Vector2 p, h_Vector2 size, u32 color) {
	p -= camera->position;
	h_Vector2Int p0((int)floor(p.x), (int)floor(p.y));
	h_Vector2Int p1((int)floor(p.x + size.x), (int)floor(p.y + size.y));

	draw_rect_pixel(p0, p1, color);
}

static void
draw_cir(h_Vector2 p, int radius, u32 color) {
	p -= camera->position;
	h_Vector2Int p0((int)floor(p.x), (int)floor(p.y));
...
static void
draw_oval(h_Vector2 p, h_Vector2Int size, u32 color) {
	p -= camera->position;
	h_Vector2Int p0((int)floor(p.x), (int)floor(p.y));
...
static void
draw_tri(h_Vector2 p0, h_Vector2 p1, h_Vector2 p2, u32 color) {
	p0 -= camera->position;
	p1 -= camera->position;
	p2 -= camera->position;
	h_Vector2Int p0_1((int)floor(p0.x), (int)floor(p0.y));
	h_Vector2Int p1_1((int)floor(p1.x), (int)floor(p1.y));
	h_Vector2Int p2_1((int)floor(p2.x), (int)floor(p2.y));
...
static void
draw_text(h_Vector2 pos, const char* text, float scale) {
	float org_y = pos.y;
	...
	while (*text) {
		...
		if (*text == 10) {
			...
			for (int i = 0; i < 7; i++) {
				...
				while (*row) {
					if (*row == '0') {
						draw_rect(h_Vector2(pos.x, pos.y), h_Vector2(scale, scale), 0xFFFFFF);
					}
					pos.x += scale;
					_width++;
					row++;
...
static void
draw_image(Image* image, h_Vector2 p, float scale, float rotation, h_Vector2 pivot) {
	p -= camera->position;
	if (outside_screen(p, h_Vector2(image->w * scale, image->h * scale))) return;
	h_Vector2Int pivint((int)floor(pivot.x), (int)floor(pivot.y));
	draw_image_pixel(image, h_Vector2Int((int)floor(p.x), (int)floor(p.y)), scale, rotation, pivint);
}

static void
draw_image(Object o) {
	o.position -= camera->position;
	if (outside_screen(o.position, h_Vector2(o.image->w * o.size, o.image->h * o.size))) return;
	h_Vector2Int pivint((int)floor(o.getPivAbs().x), (int)floor(o.getPivAbs().y));
	draw_image_pixel(o.image, h_Vector2Int((int)floor(o.position.x), (int)floor(o.position.y)), o.size, o.rotation, pivint);
}
